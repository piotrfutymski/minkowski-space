...
const IDENTITY_TRANSFORM: MVector<MVector<f64>> = MVector {
    pos: Vector2D {
        x: MVector {
            pos: Vector2D { x: 1.0, y: 0.0 },
            time: 0.0,
        },
        y: MVector {
            pos: Vector2D { x: 0.0, y: 1.0 },
            time: 0.0,
        },
    },
    time: MVector {
...
impl MVector<f64> {
    ...
    pub fn is_time_or_light_like(&self) -> bool {
        matches!(
            self.causal_character(),
            Causality::TimeLike | Causality::LightLike
        )
    }
    ...
    pub(crate) fn lorentz_transform_matrix_with_precalculated_gamma(
        ...
        let matrix_element =
            gamma_over_v_length_squared * vx_vy - one_over_v_length_squared * vx_vy;
        MVector {
            pos: Vector2D {
                x: MVector {
                    pos: Vector2D {
                        x: gamma_over_v_length_squared * vx_squared
                            + one_over_v_length_squared * vy_squared,
                        y: matrix_element,
                    },
                    time: min_vx_gamma,
                },
                y: MVector {
                    pos: Vector2D {
                        x: matrix_element,
                        y: gamma_over_v_length_squared * vy_squared
                            + one_over_v_length_squared * vx_squared,
                    },
                    time: min_vy_gamma,
                },
            },
...
impl From<&MVector<f64>> for Vector2D<f64> {
    fn from(value: &MVector<f64>) -> Self {
        value.pos
    }
}
